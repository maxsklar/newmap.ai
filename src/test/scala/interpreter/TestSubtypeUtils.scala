...
class TestSubtypeUtils extends FlatSpec {
  "Subtype " should " be convertible into the parent type" in {
    val isMember = MapInstance(
      Vector(
        ObjectPattern(IdentifierInstance("key")) -> ObjectExpression(IndexValue(1, Index(2))),
        ObjectPattern(IdentifierInstance("value")) -> ObjectExpression(IndexValue(1, Index(2)))
      ),
