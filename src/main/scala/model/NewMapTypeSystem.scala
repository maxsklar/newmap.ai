...
case class NewMapTypeSystem(
  ...
  def typeToUntaggedObject(nType: NewMapType): UntaggedObject = nType match {
    ...
    case UndefinedT => UCase(UIdentifier("Undefined"), UStruct(Vector.empty))
  ...
  def getParameterType(
    ...
    case "Undefined" => Success(emptyStructType)
  ...
  def convertToNewMapType(uType: UntaggedObject): Outcome[NewMapType, String] = uType match {
...
object NewMapTypeSystem {
  ...
  // IndexT(0)
  // Custom: CaseType / Parameterized Case Type (store)
  // Subtype(type)
  ...
  def createNewCustomType(
    ...
    val newCustomTypeId: TypeId = java.util.UUID.randomUUID
    val newTypeSystemId: Id = java.util.UUID.randomUUID
    ...
    for {
      ...
      // TODO: implement
      val newTypesThatAreUsed = typeSystem.typesThatUseType

      typeSystem.copy(
        currentState = newTypeSystemId,
        historicalMapping = typeSystem.historicalMapping + (newTypeSystemId -> (currentMapping + (name -> newCustomTypeId))),
        typeToParameterType = typeSystem.typeToParameterType + (newCustomTypeId -> parameterType),
        typeToUnderlyingType = typeSystem.typeToUnderlyingType + (newCustomTypeId -> (parameterPattern, startingPoint)),
  ...
  def upgradeCustomType(
    ...
    val newTypeId: TypeId = java.util.UUID.randomUUID
    val newTypeSystemId: Id = java.util.UUID.randomUUID
    ...
    for {
      ...
      currentParameterType <- Outcome(typeSystem.typeToParameterType.get(currentTypeId), s"Couldn't find current type parameter for $name")
      ...
      typeSystem.copy(
        currentState = newTypeSystemId,
        historicalMapping = typeSystem.historicalMapping + (newTypeSystemId -> newMapping),
        typeToParameterType = typeSystem.typeToParameterType + (newTypeId -> currentParameterType),
        typeToUnderlyingType = typeSystem.typeToUnderlyingType + (newTypeId -> (currentParameterPattern -> newUnderlyingType)),
        customConvertibilityRules = typeSystem.customConvertibilityRules + ((currentTypeId -> newTypeId) -> converter),
        typesThatUseType = newTypesThatAreUsed
      )
    }
  }
